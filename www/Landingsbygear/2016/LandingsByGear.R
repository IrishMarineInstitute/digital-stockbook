   ...
   from(
      ...
      from Declarations as d 
         ...
         left join Stockman2015_views..IntercatchStocks st
            on st.Areas = coalesce(i.IcesDivisionCorrect,d.Division)
...
###################################################### Saithe in VII ###############################################
    ...
    on s.ScientificName = sl.ScientificName
    where v.VesselProvenance='Ireland' 
    and (FSS_Speciesname like 'Saithe')and coalesce(i.IcesDivisionCorrect,d.Division) in ('VIIa','VIIb','VIIc','VIId','VIIe','VIIf','VIIg','VIIh','VIIj','VIIk')
