...
setwd('\\\\galwayfs03\\fishdata\\StockBooks\\_StockBook2019\\Plots\\LandingsByGear\\Nephrops')
...
#scale due to unallocated catches in 2018 /FU16

dat <- read.csv("nep_fus_landings.csv")


lan <- met %>% filter(Year=="2018") %>% group_by(FU) %>% summarise(tot=sum(LiveWt)/1000)
...
lan1 <- lan1 %>% group_by(FU) %>% mutate(scale=X2018/tot)

scale <-lan1[ , c(1,  7)]

#2018 only
land=filter(met, Year=="2018")
