...
setwd('\\\\galwayfs03\\fishdata\\StockBooks\\_StockBook2019\\Plots\\LandingsByGear')
...
t <- with(subset(land,stock==s), tapply(landWt,gear,sum))
t <- ifelse(is.na(t),0,t)
png(paste0(s,'.png'),2,1.75,'in',8,'white',600)
  par(mar=c(4,8.5,2,1))
  barplot(sort(t)/1000,xlim=extendrange(c(0,max(t)/1000),f=0.02),horiz=T,col=pal[7],border=pal[9],space=0.5,las=1,xlab='Tonnes',main = 'Irish landings by gear type                       ')
  box()
dev.off()
...
# manual fix of her 7aS and 7aN see herring.xlsx
# NEED TO UPDATE EVERY YEAR!!!!
s <- 'her.27.irls'
t <- as.table(c('Gillnets'=0,
                'Beam trawls'=0,
                'Midwater trawls'=4196.593,
                'Bottom otter trawls'=0.191,
                'Seines'=0,
                'Others'=3.824)) * 1000
png(paste0(s,'.png'),4,1.75,'in',8,'white',600)
  par(mar=c(4,8.5,2,1))
...
t <- as.table(c('Gillnets'=0,
                'Beam trawls'=0,
                'Midwater trawls'=1858.476,
                'Bottom otter trawls'=0,
                'Seines'=0,
                'Others'=0)) * 1000
png(paste0(s,'.png'),4,1.75,'in',8,'white',600)
  par(mar=c(4,8.5,2,1))
