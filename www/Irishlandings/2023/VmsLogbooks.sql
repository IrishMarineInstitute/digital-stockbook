...
where a.OperationDate between '1 jan 2017' and '1 jan 2022'
...
where a.date_time_logged between '1 jan 2017' and '1 jan 2022'
   and a.posn_long between -20 and 7
   and a.posn_lat between 36 and 65
   and b.estimated_speed between 
      case when rtrim(substring(coalesce(LogbookMainGear,EuMainGear),1,3)) in 
        --passive gears, lines and seines
        ('FPO','GEN','GN','GNC','GND','GNF','GNS','GTN','GTR',
        'LHM','LHP','LLD','LLS','LTL','LX','LL',
        'PS','PS1','PS2','SDN','SPR','SSC','LA','SB','SX','SV') then 0.1 
        -- all other gears (trawls and dredges and NA)
         else 0.5 end  
      and case when rtrim(substring(coalesce(LogbookMainGear,EuMainGear),1,3)) in 
         --dredges, bottom trawls
         ('DRB','DRH','HMD','DRM','OTB','OTT','PTB','TBN','OT','OTP','TB','TBS','TX') then 5.5
         when rtrim(substring(coalesce(LogbookMainGear,EuMainGear),1,3)) in 
         -- midwater trawls, longlines, beamtrawls
         ('OTM','PTM','TM','PT','LHM','LHP','LLD','LLS','LTL','LX','LL','TBB') then 6
         -- everything else (seine, gillnet and pots)
         else 4.5
         end -- only 'fishing speeds' - see page 5 of Atlas of Commercial Fisheries around Ireland 2019
   and b.dt > 0
