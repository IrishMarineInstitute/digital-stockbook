...
#data(coast)
eezWorld <- readOGR("F:/StockBooks/Stock Books Historic/StockBook 2018/Maps/VMS/PropSpecies","eez_boundaries") # marineregions v9
eezWorld1 <- subset(eezWorld,Line_type%in%c('Treaty','200 NM','Connection line','Median line','Unilateral claim (undisputed)'))
rm(eezWorld)

setwd('F:/StockBooks/_StockBook2020/maps/VMS/2015-18')

 channel <- odbcDriverConnect("Driver=SQL Server; Server=VMFSSDEV02; Database=Logbooks_20200309")
   sqlQuery(channel,readChar("VmsLogbooks.sql",10^6))
   vms <- sqlQuery(channel,readChar("RegularGrid.sql",10^6))
 close(channel)
 save(vms,file='LandingsEffort2015-18.Rdata')

#load('LandingsEffort2015-18.Rdata')
...
#  if(sp%in%c('Alb','Bft','Boc','Whb','Her','Mac','Jax','Spr','Bft','Pil','Swo')) {byx <- 0.18;  byy <- 0.12} else {byx <- 0.09;  byy <- 0.06}
  ...
  vms$land <- vms[,paste0('LiveWt',sp)] / (cos(vms$Lat*pi/180)*byx*60*1.852*byy*60*1.852)/nyears # mean land per km2 per year
  ...
  png(paste0('Land',toupper(sp),'.png'),2.75,3.25,'in',6,res=600) #3.5,4,'in',6,res=600)
 ...
 plot(eezWorld1,lwd=0.25,add=T,col='grey')
    map('worldHires',fill=T,col=col.land,border=1, lwd=0.25, add=T)
    title <- expression(paste('kg/',km^2))
    if(sp%in%c('Alb','Bft')) x <- 'topleft' else x <- 'bottomright'
    legend.grid0(x,breaks=breaks1,col=col1,title=title,inset=0.02,bg=col.sea,text.col='black',box.col='black')
  dev.off()
